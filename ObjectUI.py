...
class GeometryObjectUI(ObjectUI):
    ...
    def __init__(self, parent=None):
        ...
        # Feedrate (G94: project units per minute — mm/min or in/min)
        frlabel = QtWidgets.QLabel('Feed Rate (unit/min):')
        frlabel.setToolTip(
            "Cutting speed in the XY plane.\n"
            "Units follow the project units:\n"
            "  • MM project → mm/min (G21 + G94)\n"
            "  • IN project → in/min (G20 + G94)\n"
            "Example: 100 with a metric project means 100 mm/min."
        )
        ...
        # Spindlespeed (optional RPM; blank → M03 without S)
        spdlabel = QtWidgets.QLabel('Spindle (RPM, optional):')
        spdlabel.setToolTip(
            "Spindle speed in RPM (optional).\n"
            "• If set: emits M03 S<rpm>\n"
            "• If left blank: emits M03 only\n"
            "  (spindle ON, no speed code).\n"
            "Use blank for mills with simple\n"
            "spindle on/off (no S-word support)."
        )
...
class GerberObjectUI(ObjectUI):
    ...
    def __init__(self, parent=None):
        ...
        overlabel.setToolTip(
            ...
            "to overlap each pass. Unitless —\n"
            "does not change with mm/in."
        )
